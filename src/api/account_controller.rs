use crate::{
    ...
    models::{
        ...
        user::{LoginDTO, UserDTO, UserEdit},
    },
...
pub async fn find_all(pool: web::Data<Pool>) -> Result<HttpResponse> {
    match account_service::get_user(&pool) {
        Ok(user) => Ok(HttpResponse::Ok().json(ResponseBody::new("Success", user))),
        Err(err) => Ok(err.response()),
    }
}
...
pub async fn find_by_id(id: web::Path<String>, pool: web::Data<Pool>) -> Result<HttpResponse> {
    match account_service::get_user_by_id(id.into_inner().parse::<i32>().unwrap(), &pool) {
...
// PUT api/user/{id}
pub async fn update(id: web::Path<String>, updated_user: web::Json<UserEdit>, pool: web::Data<Pool>) -> Result<HttpResponse> {
    match account_service::update(id.into_inner().parse::<i32>().unwrap(), updated_user.0, &pool) {
...
pub async fn delete(id: web::Path<String>, pool: web::Data<Pool>) -> Result<HttpResponse> {
    match account_service::delete(id.into_inner().parse::<i32>().unwrap(), &pool) {
        Ok(()) => Ok(HttpResponse::Ok().json(ResponseBody::new(constants::MESSAGE_OK, constants::EMPTY))),
        Err(err) => Ok(err.response()),
    }
}
